...
The video above shows how we deal with pressure on curved surface, we basically "divide and conqure" by decoupling the \\( x \\) and \\( y \\) components. For horizontal component \\( P_x \\), we can treat it as if it's a vertical plate, where \\( P_x = \rho g h_{CG}\\). For the vertical component \\( P_y \\), we need the weight of the fluids above the solid boundary \\( P_y= \rho g \\) <del>V</del>\\( _{above} \\).
...
The 2nd video shows an inverted scenario, remember pressure from liquid is always acting on a surface no matter the geometry. For these upside down ish scenarios, we use a virtual/imaginary water method for \\( P_y \\), this can be done by subtracting the base geometry (often squares or rectangles). We do this again when we are taking the moment.
